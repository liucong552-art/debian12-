cat >/root/setup_wg_nat_vps_v4.sh <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail
...
# 然后拿到输出的 VPS_WG_PUB，去 NAT 机执行：bash /root/setup_nat_wg_exit_final.sh <VPS_IP> '<VPS_WG_PUB>'
# 再回到 VPS 执行：/usr/local/sbin/wg_nat_set_peer.sh '<NAT_WG_PUB>'
#
# 可覆盖参数：
WG_IF="${WG_IF:-wg-nat}"
...
[Peer]
PublicKey = ${NAT_PUB}
# 关键：NAT 出口模式必须允许 0.0.0.0/0（否则目的地址不匹配会被丢）
...
WG_IF="${WG_IF:-wg-nat}"
MARK_RAW="${MARK:-2333}"          # 允许 2333 / 0x91d
TABLE_ID="${TABLE_ID:-100}"
...
echo "======================================================"
echo
echo "下一步：去 NAT 机运行：bash /root/setup_nat_wg_exit_final.sh <VPS_IP> '${VPS_PUB}'"
EOF
