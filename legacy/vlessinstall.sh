...
# - nftables TCP 上行配额系统（自动持久化 + 1 分钟保存快照）
...
while :; do
  ...
  if grep -R "PORT=${PORT}" "${XRAY_DIR}"/vless-temp-*.meta 2>/dev/null >/dev/null; then
    USED=1
  fi
  ...
  PORT=$((PORT+1))
  if (( PORT > 50050 )); then
    echo "❌ 在 40000-50050 范围内没有空闲 TCP 端口了。" >&2
    exit 1
  fi
done
...
SERVER_ADDR=$(curl -4fsS https://ifconfig.me 2>/dev/null || hostname -I | awk '{print $1}')
NOW=$(date +%s)
EXP=$((NOW + D))
...
4) TCP 上行配额（nftables + 1 分钟保存 counters，对应 VLESS 端口）：
