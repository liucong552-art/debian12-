...
# =========================
# XrayR Node One-Key Script (stable,坑位修复版)
# - 适配 NewV2board / Xboard 的 UniProxy
# - 关键修复：
#   1) REALITYConfigs / CertConfig nil panic 兜底（配置 + 源码双保险）
#   2) users is null 不再触发 panic（v2board.go + panel.go）
#   3) token 既走 Header(Token) 也可选走 Query(token)（兼容 validation.required）
#   4) systemd + runner：避免 StartLimit 把服务打死
#   5) NodeType 固定 Vless（避免 Server does not exist）
# =========================

# ---- CRLF 自愈：如果你从 Windows 复制/Notepad 保存导致 ^M，这里会自动修复并重启脚本 ----
...
apply_patches(){
  ...
  # 5) token 兼容：在所有 SetHeader("Token", xxx) 后面补 SetQueryParam("token", xxx)
  #    ⚠️ 注意：token 走 query 可能会出现在访问日志里（Xboard 的 UniProxy 常见就是要求 query token）
  if [[ "${XR_DISABLE_TOKEN_QUERY_PATCH:-0}" != "1" ]]; then
    ...
    if [[ -n "${files}" ]]; then
      # 只在后面没跟 token query 的情况下插入，避免重复插
...
build_and_install(){
  ...
  # 国内可选：export GOPROXY=https://goproxy.cn,direct
...
Nodes:
  - PanelType: "NewV2board"
    ...
    ControllerConfig:
      ...
      # 用面板下发 REALITY（Reality/Vision 节点常用）
      DisableLocalREALITYConfig: true

      # 兜底：避免 REALITYConfigs nil panic
      REALITYConfigs:
        Show: false

      # 兜底：避免 CertConfig nil panic（用 REALITY 时一般不依赖本机证书）
      CertConfig:
        CertMode: none
EOF
...
write_runner(){
  log "安装 xrayr-runner（自动重启，不会被 StartLimit 打死）..."
...
health_check() {
  ...
  # 先试 header；不行再试 query token（兼容 validation.required）
  local code
...
main(){
  ...
  # UpdatePeriodic default + guard
  if [[ -z "${UPD:-}" ]]; then
    UPD="10"
  fi
  if ! [[ "${UPD}" =~ ^[0-9]+$ ]]; then
    die "UpdatePeriodic 必须是数字"
  fi
  if (( UPD < 10 )) && [[ "${ALLOW_FAST_UPDATE:-0}" != "1" ]]; then
    warn "你填的 UpdatePeriodic=${UPD} 太小（之前踩过 5 会放大 users is null / 启动阶段崩溃概率）。已自动改为 10。"
    UPD="10"
  fi

  # 端口建议（避开 80/443），需要你去面板把该节点端口设置成它
  local port
  port="$(pick_port)"
  ...
  log "完成 ✅"
  echo "------------------------------------------------------------"
  echo "[重要] 请到面板把该节点的 server_port / 连接端口设置为：${port}"
  echo "      （XrayR 的监听端口由面板下发；你不改面板，这里选端口也没意义）"
  echo
  echo "[排坑提示]"
  echo "  1) 如果日志里出现 users is null：现在不会再 panic，runner 会扛住并自动重试。"
  echo "  2) 如果看到 Server does not exist：99% 是 NodeType/NodeID 不匹配；本脚本已固定 NodeType=Vless。"
  echo "  3) 如果用户列表一直 0：检查面板侧 NodeID 是否正确、节点分组/套餐分组是否匹配。"
  echo "------------------------------------------------------------"

  post_check

  log "监听端口检查（你应当看到 :${port} 在监听；如果没看到，优先检查面板是否下发该端口）"
