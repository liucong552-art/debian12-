...
render_row() {
  ...
  printf '%-32s %-9s %-6s %-5s %-6s %-10s %-10s %-10s %-7s %-12s %-19s\n' \
...
printf '%s\n' '=== VLESS AUDIT ==='
print_sep
render_row "NAME" "STATE" "PORT" "RDY" "QSTAT" "LIMIT" "USED" "LEFT_Q" "USE%" "TTL_LEFT" "EXPIRE(CN)"
print_sep
...
if systemctl list-unit-files "$MAIN_VLESS" >/dev/null 2>&1; then
  ...
  render_row "$MAIN_VLESS" "$MAIN_STATE" "$MAIN_PORT" "$MAIN_READY" "none" "-" "-" "-" "-" "-" "-"
fi
