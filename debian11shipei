...
# - Hy2 临时节点 + 审计 + GC（绝对时间 TTL）
# - nftables UDP 上行配额系统（自动持久化 + 1 分钟保存快照）
#
# 已核对的完整版本，可直接替换 GitHub 上的同名脚本。

set -euo pipefail

REPO_BASE="https://raw.githubusercontent.com/liucong552-art/debian12-/main"
UP_BASE="/usr/local/src/debian11-upstream"
...
check_debian11() {
  ...
  if [[ "$codename" != "bullseye" ]]; then
    echo "❌ 本脚本仅适用于 Debian 11 (bullseye)，当前: ${codename:-未知}"
    exit 1
  fi
}
...
check_debian11() {
  ...
  codename=$(grep -E "^VERSION_CODENAME=" /etc/os-release 2>/dev/null | cut -d= -f2)
  if [ "$codename" != "bullseye" ]; then
    echo "❌ 本脚本仅适用于 Debian 11 (bullseye)，当前为: ${codename:-未知}"
    exit 1
  fi
}

check_debian11
echo "🚀 开始系统更新 (Debian 11 / bullseye)..."
...
check_debian11() {
  ...
  codename=$(grep -E "^VERSION_CODENAME=" /etc/os-release 2>/dev/null | cut -d= -f2)
  if [ "$codename" != "bullseye" ]; then
    echo "❌ 仅支持 Debian 11 (bullseye)，当前: ${codename:-未知}"
    exit 1
  fi
}
...
# 1) 如果有 meta 且没有 FORCE=1，就检查是否已过期
...
########################################
# 2) 绝对时间 TTL 运行包装脚本：hy2_run_temp.sh
#    每次启动前按 EXPIRE_EPOCH 计算剩余秒数，用 timeout 控制
########################################
...
# ------------------ 5. nftables 配额系统（1 分钟保存 counters） ------------------
...
nft -a list chain inet portquota down_out 2>/dev/null | awk '
  /handle/ && /udp sport/ {
    port=$6; handle=$NF;
    gsub(/[^0-9]/,"",port);
    printf("Port: %-6s  Handle: %-6s\n", port, handle);
  }'
AUD
  chmod +x /usr/local/sbin/pq_audit.sh

  # 定期保存 nft 规则（包括 counters）到 /etc/nftables.conf（1 分钟一次）
...
- 每次 add/del 会立即保存 nft 规则到 /etc/nftables.conf
- 额外有 pq-save.timer 每 1 分钟自动保存快照，
  重启后会从最近快照恢复，已使用量最多丢失约 1 分钟内的统计
...
4) Hy2 临时节点 + 审计 + GC（绝对时间 TTL）：
   ...
   # 部署后：
   D=600 hy2_mktemp.sh     # 新建 600 秒临时节点（按绝对时间生效）
   hy2_audit.sh            # 查看全部节点状态（剩余时间按 EXPIRE_EPOCH 计算）
   hy2_clear_all.sh        # 手动强制清空所有临时节点

5) UDP 上行配额（nftables + 1 分钟保存 counters）：
